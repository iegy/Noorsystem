...
const UI = (() => {
    ...
    function buildOrderForm() {
        ...
        if (!container) return;
        const types = window.APP_CONFIG?.ORDER_TYPES || ['نظارة وعدسات جديدة', 'عدسات فقط', 'صيانة فقط'];
        const statuses = window.APP_CONFIG?.ORDER_STATUSES || ['في المكتب', 'بالمحل'];
        const deliveries = window.APP_CONFIG?.DELIVERY_STATUSES || ['لم يتم', 'تم التسليم'];
        container.innerHTML = `
            <div class="odoo-card rounded-xl overflow-hidden">
                <div class="flex items-center justify-between p-4 sm:p-5 cursor-pointer select-none" onclick="UI.toggleOrderForm()">
                    ...
                    <div class="p-4 sm:p-5 border-t border-slate-100 dark:border-slate-600">
                        <form id="orderForm" class="space-y-4 sm:space-y-5">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                                ...
                                <div><label class="block text-sm sm:text-base font-semibold text-slate-600 dark:text-slate-300 mb-1 sm:mb-2">تصنيف العمل</label><select id="orderType" class="w-full odoo-input">${types.map(t => `<option value="${t}">${t}</option>`).join('')}</select></div>
                                <div><label class="block text-sm sm:text-base font-semibold text-slate-600 dark:text-slate-300 mb-1 sm:mb-2">التواجد</label><select id="orderStatus" class="w-full odoo-input">${statuses.map(s => `<option value="${s}">${s}</option>`).join('')}</select></div>
                                <div><label class="block text-sm sm:text-base font-semibold text-slate-600 dark:text-slate-300 mb-1 sm:mb-2">تسليم العميل</label><select id="deliveryStatus" class="w-full odoo-input">${deliveries.map(d => `<option value="${d}">${d === 'لم يتم' ? 'لم يتم ❌' : 'تم التسليم ✅'}</option>`).join('')}</select></div>
    ...
    function renderTable() {
        ...
        displayOrders.forEach(o => {
            ...
            visibleCols.forEach(col => {
                switch(col.id) {
                    ...
                    case 'status':
                        const badge = o.status==='في المكتب'?'bg-amber-50 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300':'bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300';
                        rowHtml += `<td class="p-3 sm:p-4 text-center"><span class="inline-flex items-center gap-1 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold ${badge}"><span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${o.status==='في المكتب'?'bg-amber-500':'bg-emerald-500'}"></span>${o.status}</span></td>`; break;
                    case 'delivery':
                        const d = o.delivery==='تم التسليم'?'<span class="text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-900/30 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm"><i class="fa-solid fa-check"></i> تم</span>':'<span class="text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm"><i class="fa-solid fa-clock"></i> لم يتم</span>';
                    ...
                    case 'notes': rowHtml += `<td class="p-3 sm:p-4 max-w-[100px] sm:max-w-[140px] truncate text-slate-500 dark:text-slate-400" title="${(o.notes||'').replace(/"/g, '&quot;')}">${o.notes||'—'}</td>`; break;
                    case 'days':
                        let daysHtml = days;
            ...
            if (isAdmin) {
                rowHtml += `<td class="p-3 sm:p-4 text-center no-print"><div class="flex justify-center gap-1">
                    <button onclick="OrdersModule.openEditModal('${o.id}')" class="text-sky-600 hover:bg-sky-50 dark:hover:bg-sky-900/30 p-1.5 sm:p-2 rounded-lg" title="تعديل"><i class="fa-solid fa-pen-to-square text-base sm:text-lg"></i></button>
                    <button onclick="OrdersModule.toggleStatus('${o.id}')" class="text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 p-1.5 sm:p-2 rounded-lg" title="تبديل التواجد"><i class="fa-solid fa-right-left text-base sm:text-lg"></i></button>
                    <button onclick="OrdersModule.deleteOrder('${o.id}')" class="text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 p-1.5 sm:p-2 rounded-lg" title="حذف"><i class="fa-regular fa-trash-can text-base sm:text-lg"></i></button>
                </div></td>`;
            }
        ...
        if (reportsTab && !reportsTab.classList.contains('hidden') && typeof ReportsModule !== 'undefined') {
    ...
    function getFilteredOrders() {
        ...
        if (user?.role === 'super_admin') {
            if (currentBranch === 'all') branchOrders = [...FirebaseModule.orders];
    ...
    return {
        init, renderTable, filterTable, searchOrders, goToPage,
        openColumnSettings, saveColumnSettings, updateColumnCount,
        closeModal, toggleDarkMode, toggleOrderForm,
